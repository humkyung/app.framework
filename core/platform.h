...
/************[ Epsilon constants ]*************/
const double Epsilon_High      = 1.0E-16;
const double Epsilon_Medium    = 1.0E-10;
const double Epsilon_Low       = 1.0E-07;
const double EpsilonV          = Epsilon_Medium;
...
template<typename T>
inline bool equals(const T& val1, const T& val2, const T& eps = std::numeric_limits<T>::epsilon())
{
	T diff = val1 - val2;
	assert(((-eps <= diff) && (diff <= eps)) == (abs(diff) <= eps));
